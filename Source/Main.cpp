...
static juce::Font makeFont(float height, const juce::String& style)
{
    auto name = juce::Font::getDefaultSansSerifFontName();
    auto options = juce::FontOptions().withName(name).withStyle(style).withHeight(height);
...
public:
    ...
    {
        g.fillAll(juce::Colour(0xff0f1114));
        ...
        for (int i = 0; i < timelineCount; ++i)
        {
            ...
            auto rowFill = (i % 2 == 0) ? juce::Colour(0xff171c22) : juce::Colour(0xff14181e);
            auto rowRect = row.reduced(3, 3).toFloat();
            g.setColour(juce::Colour(0xff000000).withAlpha(0.35f));
            g.fillRoundedRectangle(rowRect.translated(0.0f, 1.0f), 6.0f);
            g.setColour(rowFill);
            g.fillRoundedRectangle(rowRect, 6.0f);
            g.setColour(juce::Colour(0xff2b323c).withAlpha(0.45f));
            g.drawRoundedRectangle(rowRect, 6.0f, 1.0f);
            ...
            {
                g.setColour(juce::Colour(0xff4aa3ff).withAlpha(0.85f));
                g.drawRoundedRectangle(rowRect, 6.0f, 2.0f);
            }

            if (i == dragOverTimelineIndex)
            {
                g.setColour(juce::Colour(0xff8ab4f8).withAlpha(0.18f));
                g.fillRoundedRectangle(rowRect.reduced(1.0f), 6.0f);
            }
            ...
            g.setColour(juce::Colour(0xffcc6f3d));
            ...
            for (int markerIndex = 0; markerIndex < markerCount; ++markerIndex)
            {
                ...
                int markerY = markerArea.getCentreY() + yOffset;
                ...
                juce::Path p;
                p.addTriangle((float) x, (float) markerY - 10.0f, (float) x - 6.0f, (float) markerY + 8.0f, (float) x + 6.0f, (float) markerY + 8.0f);
                ...
                g.setColour(juce::Colour(0xffdfe6ee));
                auto label = formatTime(marker->startTimeSeconds, timeline);
                int labelY = juce::jmax(markerArea.getY() + 2, markerY - 24);
    ...
    void mouseDown(const juce::MouseEvent& e) override
    {
        ...
        fileChooser->launchAsync(juce::FileBrowserComponent::openMode | juce::FileBrowserComponent::canSelectFiles,
                                 [this, timelineIndex, timeSeconds](const juce::FileChooser& chooser)
                                 {
                                     ...
                                     auto& timelineRef = model.getTimeline(timelineIndex);
                                     auto* marker = new Marker();
                                     marker->startTimeSeconds = juce::jlimit(0.0, timelineRef.durationSeconds, timeSeconds);
                                     marker->pythonFile = file;

                                     timelineRef.markers.add(marker);

                                     double renderDuration = getRenderDurationSecondsForMarker(timelineRef, *marker);
                                     renderer.renderMarker(*marker, sampleRateForRender, renderDuration, timelineRef.tempoBpm,
...
private:
    ...
    void drawGrid(juce::Graphics& g, juce::Rectangle<int> row, const Timeline& timeline)
    {
        ...
        if (gridMode == GridMode::Seconds)
        {
            ...
            for (; t <= end + 0.0001; t += minor)
            {
                ...
                g.setColour(isMajor ? juce::Colour(0xff667584) : juce::Colour(0xff44505c));
        ...
        else
        {
            ...
            for (; t <= end + 0.0001; t += beatSeconds)
            {
                ...
                auto x = timeToX(t, timeline, content);
                g.setColour(isBar ? juce::Colour(0xff667584) : juce::Colour(0xff44505c));
                g.drawLine((float) x, (float) content.getY(), (float) x, (float) content.getBottom());
            }
        }
    }
    ...
    void drawAutomationLanes(juce::Graphics& g, juce::Rectangle<int> row, const Timeline& timeline)
    {
        ...
        g.fillRect(lanes.volume);
        g.fillRect(lanes.pan);
        g.setColour(juce::Colour(0xff2b323c).withAlpha(0.5f));
        ...
        drawAutomationPoints(g, volPoints, timeline.volumeAutomation, timeline, 0.0, 1.0, juce::Colour(0xff34a853));
        drawAutomationPoints(g, panPoints, timeline.panAutomation, timeline, -1.0, 1.0, juce::Colour(0xff2f80ed));
    }
                            ...
                            int yOffset)
    {
        ...
        for (int i = 0; i < points; ++i)
        {
            ...
            float amp = marker.waveform[(size_t) i];
    ...
    void drawRepeatMarkers(juce::Graphics& g, juce::Rectangle<int> area, const Timeline& timeline)
    {
        ...
        if (timeline.repeatMarkers.size() >= 2)
        {
            ...
            if (loopEnd > loopStart && isTimeInView(loopStart, timeline) && isTimeInView(loopEnd, timeline))
            {
                ...
                int y = area.getY() + 2;
                g.setColour(juce::Colour(0xfff08bbb).withAlpha(0.75f));
                g.drawLine((float) x0, (float) y, (float) x1, (float) y, 2.0f);
            }
        }

        g.setColour(juce::Colour(0xffc56a9a).withAlpha(0.9f));
        for (int i = 0; i < timeline.repeatMarkers.size(); ++i)
        {
            double t = timeline.repeatMarkers.getUnchecked(i);
            if (! isTimeInView(t, timeline))
            ...
            if (selectedRepeatMarkerIndex == i && selectedRepeatMarkerTimelineIndex == selectedTimelineIndex)
            {
                ...
                g.setColour(juce::Colour(0xffe890b8).withAlpha(0.8f));
            }
            else
            {
    ...
    void drawHeader(juce::Graphics& g, juce::Rectangle<int> row, const Timeline& timeline, int index)
    {
        ...
        g.setColour(juce::Colour(0xffe2e7ef));
        g.setFont(makeFont(12.5f, "Semibold"));
    ...
    void drawAutomationToggle(juce::Graphics& g, juce::Rectangle<int> header, const Timeline& timeline)
    {
        ...
        g.setColour(juce::Colour(0xff9aa4b2));
        g.drawLine((float) (cx - 4), (float) cy, (float) (cx + 4), (float) cy, 2.0f);
        if (! timeline.automationExpanded)
            g.drawLine((float) cx, (float) (cy - 4), (float) cx, (float) (cy + 4), 2.0f);
    }
                              ...
                              juce::Colour colour)
    {
        ...
        for (const auto& p : points)
        {
            ...
            g.setColour(juce::Colour(0xff0a0d10).withAlpha(0.65f));
            ...
            if (isSelected)
            {
                ...
                g.setColour(juce::Colour(0xffeef3f9));
...
struct AppLookAndFeel : public juce::LookAndFeel_V4
{
    AppLookAndFeel()
    {
        setColour(juce::TextButton::buttonColourId, juce::Colour(0xff20242b));
        setColour(juce::TextButton::buttonOnColourId, juce::Colour(0xff2b313b));
        setColour(juce::TextButton::textColourOffId, juce::Colour(0xffe6e9ef));
        setColour(juce::TextButton::textColourOnId, juce::Colour(0xffe6e9ef));
        setColour(juce::ComboBox::backgroundColourId, juce::Colour(0xff1c2027));
        setColour(juce::ComboBox::textColourId, juce::Colour(0xffe6e9ef));
        setColour(juce::ComboBox::outlineColourId, juce::Colour(0xff2b323c));
        setColour(juce::Label::textColourId, juce::Colour(0xffc9d1dc));
        setColour(juce::Label::outlineColourId, juce::Colour(0xff2b323c));
        setColour(juce::TextEditor::backgroundColourId, juce::Colour(0xff151a20));
        setColour(juce::TextEditor::textColourId, juce::Colour(0xffe6e9ef));
        setColour(juce::TextEditor::outlineColourId, juce::Colour(0xff2b323c));
        setColour(juce::Slider::trackColourId, juce::Colour(0xff2c333d));
        setColour(juce::Slider::thumbColourId, juce::Colour(0xff2f80ed));
        setColour(juce::Slider::textBoxBackgroundColourId, juce::Colour(0xff151a20));
        setColour(juce::Slider::textBoxTextColourId, juce::Colour(0xffe6e9ef));
        setColour(juce::Slider::textBoxOutlineColourId, juce::Colour(0xff2b323c));
    }

    void drawButtonBackground(juce::Graphics& g, juce::Button& button,
                              const juce::Colour& backgroundColour, bool isHovered, bool isDown) override
    {
        auto bounds = button.getLocalBounds().toFloat().reduced(0.5f);
        auto base = backgroundColour;
        if (isDown)
            base = base.darker(0.05f);
        else if (isHovered)
                          ...
                          juce::Slider& slider) override
    {
        ...
        auto track = juce::Rectangle<float>((float) x, (float) y, (float) width, (float) height)
                         .withHeight(4.0f).withCentre({ (float) x + width * 0.5f, (float) y + height * 0.5f });
        g.setColour(slider.findColour(juce::Slider::trackColourId));
        g.fillRoundedRectangle(track, 2.0f);

        auto filled = track.withWidth(sliderPos - (float) x);
        g.setColour(juce::Colour(0xff2f80ed).withAlpha(0.9f));
    ...
    {
        return makeFont((float) buttonHeight * 0.45f, "Regular");
    }

    juce::Font getLabelFont(juce::Label& label) override
    {
        float h = label.getHeight() > 0 ? (float) label.getHeight() * 0.55f : 14.0f;
        return makeFont(h, "Regular");
    }

    juce::Font getComboBoxFont(juce::ComboBox&) override
    {
        return makeFont(14.0f, "Regular");
    }
};
...
public:
                      ...
                      [this] { updateInspectorFromModel(); })
    {
        ...
        playButton.setColour(juce::TextButton::buttonColourId, juce::Colour(0xff1c2a22));
        playButton.setColour(juce::TextButton::buttonOnColourId, juce::Colour(0xff24362c));
        ...
        stopButton.setColour(juce::TextButton::buttonColourId, juce::Colour(0xff2a2020));
        ...
        loadButton.setColour(juce::TextButton::buttonColourId, juce::Colour(0xff1f242c));
        ...
        saveButton.setColour(juce::TextButton::buttonColourId, juce::Colour(0xff1f242c));
        ...
        exportButton.setColour(juce::TextButton::buttonColourId, juce::Colour(0xff1f242c));
        ...
        addTrackButton.setColour(juce::TextButton::buttonColourId, juce::Colour(0xff202634));
        ...
        inspectorTitle.setFont(makeFont(17.0f, "Semibold"));
        ...
        validationStatus.setFont(makeFont(12.0f, "Regular"));

        addAndMakeVisible(validationResults);
        validationResults.setJustificationType(juce::Justification::topLeft);
        validationResults.setFont(makeFont(11.0f, "Regular"));
        validationResults.setColour(juce::Label::textColourId, juce::Colour(0xffd68a45));
        ...
        timelineInfoLabel.setColour(juce::Label::textColourId, juce::Colour(0xffc9d1dc));

        addAndMakeVisible(timeSigLabel);
        timeSigLabel.setText("Time Signature", juce::dontSendNotification);
        timeSigLabel.setColour(juce::Label::textColourId, juce::Colour(0xffaeb7c2));

        addAndMakeVisible(tempoLabel);
        tempoLabel.setText("Tempo (BPM)", juce::dontSendNotification);
        tempoLabel.setColour(juce::Label::textColourId, juce::Colour(0xffaeb7c2));
        ...
        durationLabel.setColour(juce::Label::textColourId, juce::Colour(0xffaeb7c2));
        ...
        markerRenderBarsLabel.setColour(juce::Label::textColourId, juce::Colour(0xffaeb7c2));
        ...
        volumeLabel.setColour(juce::Label::textColourId, juce::Colour(0xffaeb7c2));
        ...
        panLabel.setColour(juce::Label::textColourId, juce::Colour(0xffaeb7c2));
        ...
        zoomLabel.setColour(juce::Label::textColourId, juce::Colour(0xffaeb7c2));
        ...
        scrollLabel.setColour(juce::Label::textColourId, juce::Colour(0xffaeb7c2));
        ...
        zoomYLabel.setColour(juce::Label::textColourId, juce::Colour(0xffaeb7c2));
        ...
        automationZoomYLabel.setColour(juce::Label::textColourId, juce::Colour(0xffaeb7c2));
        ...
        timelineViewport.getVerticalScrollBar().setColour(juce::ScrollBar::thumbColourId, juce::Colour(0xff2b323c));
        timelineViewport.getVerticalScrollBar().setColour(juce::ScrollBar::trackColourId, juce::Colour(0xff12161c));
        timelineViewport.getHorizontalScrollBar().setColour(juce::ScrollBar::thumbColourId, juce::Colour(0xff2b323c));
        timelineViewport.getHorizontalScrollBar().setColour(juce::ScrollBar::trackColourId, juce::Colour(0xff12161c));
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        g.setGradientFill(juce::ColourGradient(juce::Colour(0xff171b21), 0, 0,
                                               juce::Colour(0xff111419), 0, (float) topBar.getBottom(), false));
        g.fillRect(topBar);
        g.setColour(juce::Colour(0xff2b323c).withAlpha(0.6f));
        g.drawLine(0.0f, (float) topBar.getBottom(), (float) getWidth(), (float) topBar.getBottom(), 1.0f);

        g.setColour(juce::Colour(0xffc9d1dc));
        g.setFont(makeFont(13.0f, "Semibold"));
        g.drawText("PyLine 2.0.1", 16, topBar.getY(), 200, topBar.getHeight(), juce::Justification::centredLeft);

        auto drawGroup = [&](juce::Rectangle<float> r)
        {
        ...
        auto transport = playButton.getBounds().getUnion(stopButton.getBounds()).toFloat().expanded(6.0f, 6.0f);
        auto fileGroup = loadButton.getBounds().getUnion(saveButton.getBounds()).getUnion(exportButton.getBounds()).toFloat().expanded(6.0f, 6.0f);
        auto gridGroup = gridModeBox.getBounds().getUnion(snapToggle.getBounds()).getUnion(scissorsToggle.getBounds()).getUnion(snapResBox.getBounds()).toFloat().expanded(6.0f, 6.0f);
        auto addGroup = addTrackButton.getBounds().toFloat().expanded(6.0f, 6.0f);
    ...
    void recomputeWaveform(Marker& marker)
    {
        ...
        for (int i = 0; i < points; ++i)
        {
            ...
            marker.waveform[(size_t) i] = juce::jlimit(0.0f, 1.0f, peak);
        }
    }
    ...
    void resized() override
    {
        ...
        exportButton.setBounds(top.removeFromLeft(90));
    ...
    struct ExportSettings
    {
        ...
        bool normalizePerStem = false;
        bool normalizeRms = false;
    };

    void showExportDialog()
    {
        auto* dialog = new juce::AlertWindow("Export Audio", "Choose export settings", juce::AlertWindow::NoIcon);
        dialog->addComboBox("scope", { "All Tracks", "Selected Track" }, "Scope");
        dialog->addComboBox("rate", { "48000", "44100" }, "Sample Rate");
        dialog->addComboBox("depth", { "24", "16" }, "Bit Depth");
        dialog->addTextEditor("length", "", "Length (seconds, blank = full)");
        dialog->addComboBox("normalize", { "No Normalize", "Normalize" }, "Normalize");
        dialog->addComboBox("normmode", { "Peak", "RMS" }, "Normalize Mode");
        dialog->addComboBox("perstem", { "Whole Mix", "Per Stem" }, "Normalize Scope");
        dialog->addComboBox("stems", { "Single Mixdown", "Export Stems" }, "Stems");
        ...
        dialog->enterModalState(true,
                                ...
                                {
                                    std::unique_ptr<juce::AlertWindow> guard(dialog);
                                    if (result != 1)
                                        return;

                                    ExportSettings settings;
                                    settings.exportAllTracks = (dialog->getComboBoxComponent("scope")->getSelectedId() == 1);
                                    settings.sampleRate = dialog->getComboBoxComponent("rate")->getText().getDoubleValue();
                                    settings.bitDepth = dialog->getComboBoxComponent("depth")->getText().getIntValue();
                                    settings.lengthSeconds = dialog->getTextEditor("length")->getText().getDoubleValue();
                                    settings.normalize = (dialog->getComboBoxComponent("normalize")->getSelectedId() == 2);
                                    settings.normalizeRms = (dialog->getComboBoxComponent("normmode")->getSelectedId() == 2);
                                    settings.normalizePerStem = (dialog->getComboBoxComponent("perstem")->getSelectedId() == 2);
                                    settings.exportStems = (dialog->getComboBoxComponent("stems")->getSelectedId() == 2);

                                    if (! settings.exportAllTracks)
                                    {
                                        int selected = selectedTimelineIndex;
                                        if (selected < 0 || selected >= model.getTimelineCount())
                                        {
                                            juce::AlertWindow::showMessageBoxAsync(juce::AlertWindow::WarningIcon, "Export Audio",
                                                                                  "No selected track to export.");
                                            return;
                                        }
                                        settings.trackIndex = selected;
                                    }
                                    ...
                                    exportChooser = std::make_unique<juce::FileChooser>("Export WAV", juce::File{}, "*.wav");
                                    exportChooser->launchAsync(juce::FileBrowserComponent::saveMode | juce::FileBrowserComponent::canSelectFiles,
                                                               [this, settings](const juce::FileChooser& chooser) mutable
                                                               {
                                                                   auto file = chooser.getResult();
                                                                   if (file == juce::File())
                                                                       return;
                                                                   ...
                                                                   if (settings.exportStems && settings.exportAllTracks)
                                                                       renderExportStems(file, settings);
                                                                   else
                                                                       renderExportToFile(file, settings);
                                                               });
                                }));
    }

    double getExportLengthSeconds(const ExportSettings& settings) const
    {
        if (settings.lengthSeconds > 0.0)
            return settings.lengthSeconds;
        if (! settings.exportAllTracks && settings.trackIndex >= 0 && settings.trackIndex < model.getTimelineCount())
            return model.getTimeline(settings.trackIndex).durationSeconds;
        double maxLen = 0.0;
        for (int i = 0; i < model.getTimelineCount(); ++i)
            maxLen = juce::jmax(maxLen, model.getTimeline(i).durationSeconds);
        return maxLen;
    }

    float computePeakForBuffer(const juce::AudioBuffer<float>& buffer, int numSamples) const
    {
        float peak = 0.0f;
    ...
    float computeRmsForBuffer(const juce::AudioBuffer<float>& buffer, int numSamples) const
    {
        double sum = 0.0;
        int channels = buffer.getNumChannels();
        for (int ch = 0; ch < channels; ++ch)
        ...
        double denom = juce::jmax(1, numSamples * channels);
        return (float) std::sqrt(sum / denom);
    }

    void applyNormalization(juce::AudioBuffer<float>& buffer, int numSamples, bool useRms)
    {
        float level = useRms ? computeRmsForBuffer(buffer, numSamples)
                             : computePeakForBuffer(buffer, numSamples);
        if (level <= 0.0f)
            return;
        float target = useRms ? 0.2f : 0.99f;
        float gain = target / level;
        buffer.applyGain(0, numSamples, gain);
    }

    void renderExportToFile(const juce::File& file, const ExportSettings& settings)
    {
        double lengthSeconds = getExportLengthSeconds(settings);
        if (lengthSeconds <= 0.0)
            return;

        int totalSamples = (int) std::round(lengthSeconds * settings.sampleRate);
        if (totalSamples <= 0)
            return;
        ...
        {
            int num = juce::jmin(blockSize, totalSamples - written);
            block.clear();

            for (int i = 0; i < num; ++i)
            {
                double time = (double) (written + i) / settings.sampleRate;
                float left = 0.0f;
                float right = 0.0f;

                int tStart = 0;
                int tEnd = model.getTimelineCount();
                if (! settings.exportAllTracks)
                {
                    tStart = settings.trackIndex;
                    tEnd = settings.trackIndex + 1;
                }
            ...
            fullBuffer.copyFrom(0, written, block, 0, 0, num);
            fullBuffer.copyFrom(1, written, block, 1, 0, num);
            written += num;
        }

        if (settings.normalize)
            applyNormalization(fullBuffer, totalSamples, settings.normalizeRms);
        ...
        writer->writeFromAudioSampleBuffer(fullBuffer, 0, totalSamples);
    }

    void renderExportStems(const juce::File& file, const ExportSettings& settings)
    {
        ...
        for (int t = 0; t < model.getTimelineCount(); ++t)
        {
            ...
            stemSettings.exportAllTracks = false;
            stemSettings.trackIndex = t;
            if (! settings.normalizePerStem)
                stemSettings.normalize = false;
            juce::String stemName = base + "_track_" + juce::String(t + 1) + ".wav";
            auto stemFile = dir.getChildFile(stemName);
            renderExportToFile(stemFile, stemSettings);
        }
    }
